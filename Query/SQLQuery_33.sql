USE NORTHWND
-- Xếp hạng sản phẩm dựa trên giá giảm dần
SELECT 
	ProductID,
	ProductName,
	CategoryID,
...
-- Xếp hạng sản phẩm dựa trên giá giảm dần theo thể loại, không nhảy hạng
SELECT 
	ProductID,
	ProductName,
	CategoryID,
...
-- Xếp hạng sản phẩm dựa trên giá giảm dần theo thể loại, xếp hạng theo số thứ tự
SELECT 
	ProductID,
	ProductName,
	CategoryID,
	UnitPrice,
	ROW_NUMBER() OVER (PARTITION BY CategoryID ORDER BY UnitPrice DESC) as Ranking
FROM dbo.Products
...
-- Xếp hạng sinh viên toàn trường dựa trên điểm trung bình
SELECT 
	[ma_sinh_vien],
	[ho_ten], 
	[diem_trung_binh],
...
-- Xếp hạng sinh viên theo từng lớp học dựa trên điểm trung bình giảm dần, không bị trùng hạng
SELECT 
  	[ma_sinh_vien],
  	[ho_ten],
  	[diem_trung_binh],
  	[ma_lop_hoc],
	ROW_NUMBER() OVER (PARTITION BY [ma_lop_hoc] ORDER BY [diem_trung_binh] DESC) as xep_hang
FROM [dbo].[sinh_vien]
...
-- Tính tổng danh số bán hàng hàng năm cho mỗi khách hàng và xếp hạng khách hàng dựa trên danh số bán hàng
-- Bước 1: Tính tổng doanh thu theo từng mã đơn hàng
WITH TotalSales AS (
	SELECT
		[OrderID],
...
-- Bước 2: Join bảng tạm với bảng Orders (dựa trên OrderID) để lấy thông tin
SELECT
	[CustomerID],
	YEAR([OrderDate]) as Year,
	ts.TotalPrice,
	RANK() OVER (PARTITION BY YEAR(o.Orderdate) ORDER BY ts.TotalPrice DESC) AS Ranking
FROM TotalSales ts
JOIN dbo.Orders o
